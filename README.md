## Distributed Systems & Platform Engineer

**Tech Lead / Software Architect**

Senior architect and engineering leader with 20+ years of experience designing and delivering large-scale distributed systems and data platforms.

I combine deep hands-on engineering expertise with architectural vision, technical leadership, and a strong focus on reliable, scalable, and maintainable platforms.

## 🖥️ Slides

- *["Cassandra ... or not"](./slides/en/cassandra/) `#database` `#architecture` `#distributed` (2026/8/1 - *update*) [[🇫🇷](./slides/fr/cassandra/)]
- *[Scala 2 typeclass](./slides/en/scala2-typeclass/) `#scala` `#typeclass` (2026/8/28)
